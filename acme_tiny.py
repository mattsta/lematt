#!/usr/bin/env python
# Copyright Daniel Roesler, under MIT license, see LICENSE at
...
def get_crt(
    ...
    def _cmd(cmd_list, stdin=None, cmd_input=None,
    ...
    def _send_signed_request(url, payload, err_msg, depth=0):
        ...
        try:
            return _do_request(url, data=data.encode(
                'utf8'), err_msg=err_msg, depth=depth)
        except IndexError:  # retry bad nonces (they raise IndexError)
            return _send_signed_request(
                url, payload, err_msg, depth=(depth + 1))
    ...
    # create account, update contact details (if any), and set the global key
    # identifier
    ...
    for auth_url in order['authorizations']:
        ...
        if authorization['status'] != "valid":
            raise ValueError(
                "Challenge did not pass for {0}: {1}".format(
